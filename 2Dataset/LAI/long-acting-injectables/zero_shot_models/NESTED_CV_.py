...
class NESTED_CV:
    ...
    def __init__(self, datafile = "zero_shot_models/Dataset_14_feat.xlsx", model_type = None):
          ...
          for train_index, test_index in cv_outer.split(self.X, self.Y, self.G):
              ...
              if synth_type == 'Normal':
                  ...
                  if synth_type == 'VectGan':
                      ...
                      syn = tg.sample(n=int((1) * len(X_train)))
    ...
    def results(self, base_dir, tag, model_name):
        ...
        # ensure dir: zero_shot_models/NESTED_CV_RESULTS/<tag>/
        out_dir = os.path.join(base_dir, "NESTED_CV_RESULTS", str(tag))
        os.makedirs(out_dir, exist_ok=True)

        # filenames include synth tag and model name
        ...
        print(f"[INFO] Saved CV results CSV:  {csv_path}")
        print(f"[INFO] Saved CV results PKL:  {pkl_path}")

    def best_model(self, base_dir, tag, model_name):
        # select best params → fit full data
        ...
        # ensure dir: zero_shot_models/Trained_models/
        model_dir = os.path.join(base_dir, "Trained_models")
        os.makedirs(model_dir, exist_ok=True)

        # save model and params
        model_path = os.path.join(model_dir, f"best_model_{tag}_{model_name}.pkl")
        with open(model_path, "wb") as f:
            pickle.dump(self.best_model, f)
        print(f"[INFO] Saved best model PKL:  {model_path}")

    
def main():
    base_dir    = "zero_shot_models"
    synth_types = ['TempusGan', 'Normal', 'VectGan', 'CTGAN', 'Gaussian Copula', 'TVAE']
    all_models  = ['RF', 'MLR', 'lasso', 'kNN', 'PLS', 'SVR', 'DT', 'LGBM', 'XGB', 'NGB']
    all_results = []

    for model_name in all_models:
        for synth in synth_types:
            print(f"\n==== Running Model: {model_name} | Synth: {synth} ====")
            cv_runner = NESTED_CV(datafile="zero_shot_models/Dataset_14_feat.xlsx",
                                  model_type=model_name)
            cv_runner.input_target()
            cv_runner.cross_validation(input_value=5, synth_type=synth)

            # save per-run CV results into zero_shot_models/NESTED_CV_RESULTS/<synth>/
            cv_runner.results(base_dir=base_dir, tag=synth, model_name=model_name)

            # fit + save best model into zero_shot_models/Trained_models/
            cv_runner.best_model(base_dir=base_dir, tag=synth, model_name=model_name)
    ...
    summary = final_results.groupby(['Model','Synth_Type'])[['Test Score','Valid Score']].agg(['mean','std','min','max']).reset_index()
    pivot = final_results.pivot_table(index='Model', columns='Synth_Type', values='Test Score', aggfunc='mean')

    # (optional) also store global summaries under zero_shot_models/
    final_results.to_csv(os.path.join(base_dir, "all_nested_cv_results_zero.csv"), index=False)
    summary.to_csv(os.path.join(base_dir, "summary_by_model_synth_zero.csv"), index=False)
    pivot.to_csv(os.path.join(base_dir, "pivot_by_model_synth_zero.csv"))

    print("\nAll detailed results saved to", os.path.join(base_dir, "all_nested_cv_results_zero.csv"))
    print("Summary saved to", os.path.join(base_dir, "summary_by_model_synth_zero.csv"))
    print("Pivot table saved to", os.path.join(base_dir, "pivot_by_model_synth_zero.csv"), "\n")
