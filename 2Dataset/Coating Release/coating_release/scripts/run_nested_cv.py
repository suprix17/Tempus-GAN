...
def plot_scores(plot_data: dict[str, pd.DataFrame], score_types: list[str], data_tag:str) -> None:
    ...
    plt.tight_layout()
    plt.savefig(f"new/model_summary_grid_{data_tag}.png", dpi=300)
    plt.show()
...
def main() -> None:
    ...
    """
    model_names: list[str] = ["XGBoost", "KNN", "RF", "LightGBM"] #, "SVM"
    ...
    for i in ['TempusGan', 'Normal', 'VectGan', 'CTGAN', 'Gaussian Copula', 'TVAE']:
        print(f'\n::{i}::\n')
        ...
        for model_name in tqdm(model_names, desc="Models"):
            results_df: pd.DataFrame = nested_cross_validation(x=x, y=y, model_name=model_name, data_tag=i)
            ...
            print(np.mean(results_df["R2 Score"].tolist()), np.mean(results_df["MAE"].tolist()), np.mean(results_df["MSE"].tolist()))
        ...
        plot_scores(plot_data, ["R2 Score", "MAE", "MSE"], data_tag=i)
