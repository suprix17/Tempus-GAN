...
def main() -> None:
    ...
    """
    model_names: list[str] = ["LightGBM", "XGBoost", "KNN", "RF"]#, "SVM"
    ...
    for i in ['Normal', 'TempusGan', 'VectGan', 'CTGAN', 'Gaussian Copula', 'TVAE']:
        print(f'::{i}::')
        for model_name in tqdm(model_names, desc="Models"):
            results, predictions = evaluate_model(x=x_test_scaled, y=y_test, model_name=model_name, tag = i)
            metrics_results[model_name] = results
            test_full[f"{model_name}_pred"] = predictions

        test_full.to_csv(f"data/new_test_predictions_{i}.csv", index=False)

        metrics_df: pd.DataFrame = pd.DataFrame(metrics_results).T
        metrics_df.to_csv(f"new/valid/valid_test_scores_{i}.csv")
        ...
        for metric in ["r2", "mae", "mse"]:
            ...
            plt.tight_layout()
            plt.savefig(f"new/valid/test_{metric}_performance_{i}.png")
            plt.show()
